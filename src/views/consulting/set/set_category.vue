<template>
	<div class="forwarding-set" v-loading.body="listLoading">
		<div class="title">
			<span>카테고리 설정</span><button @click="addpopup = true"><i class="fa fa-plus-circle"></i>추가</button>
		</div>
		...
		<el-dialog :visible.sync="addpopup" width="400px" class="addpopup">
			<el-form :model="sendform" ref="sendform" :rules="rules">
				<el-form-item label="카테로리 명" prop="name">
			...
			sendForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.addpopup = false
						this.listLoading = true
						updCat(this.sendform).then(response => {
							if (valid) {
